...
function assemble(m){
  ...
  return {...site,home:{featured:(home.featured || []).map(id => {const w = works.find(w => w.id === id || base(w.file) === id || base(w.file) === id + '.jpg');return w ? base(w.file) : null;}).filter(Boolean)},pages,
    settings:{colours:look.colours,typeface:look.typeface,logo:look.logo && {style:look.logo.style,image:base(look.logo.image)},home:{showSelectedWorks:home.showSelectedWorks !== false,showCategories:home.showCategories !== false,showCommission:home.showCommission !== false,selectedCount:home.selectedCount || 6},labels:look.labels,social:look.social || [],footerLine:look.footerLine || '',components:look.components || {},blocks:look.blocks || {}},
    categories:cats.map(c => ({slug:c.slug,title:c.title,intro:c.intro || '',works:works.filter(w => w.category === c.slug).map(w => ({id:w.id,file:base(w.file),title:w.title,medium:w.medium,size:w.size || '',year:w.year || '',price:w.price || '',sold:!!w.sold}))})),thumbs};
}
...
function change(key,value){const [record,field] = key.split(':');mutate(m => {let obj = record.split('/').reduce((o,k)=>o[k],m);const parts=field.split('.');for(const p of parts.slice(0,-1)) obj=obj[p] ??= {};obj[parts.at(-1)] = value;syncPageName(m,key,value);});}
function syncPageName(m,key,value){
  const page=key.match(/^pages\/(about|commission|shop|contact):title$/);
  const label=key.match(/^settings\/look:labels\.(about|commission|shop|contact)$/);
  if(page){m.settings.look.labels ??= {};m.settings.look.labels[page[1]]=value;}
  if(label)m.pages[label[1]].title=value;
}
...
function draw(){
  const previousScroll = frame.contentWindow.scrollY;
  ...
  frame.contentWindow.scrollTo({top:previousScroll,behavior:'instant'});
}
function pageOptions(){const value=frame.contentWindow.__site.pathOf();$('#page-switcher').innerHTML=[['','Home'],['about',model.pages.about.title],...sorted(model.categories).map(([id,c])=>['portfolio/'+id,c.title]),...['commission','shop','contact'].map(k=>[k,model.pages[k].title])].map(([p,t])=>`<option value="${esc(p)}">${esc(t)}</option>`).join('');$('#page-switcher').value=value;}
...
function pictureReferences(){return [...Object.entries(model.works).map(([id,w])=>({key:'works/'+id+':file',file:w.file,title:w.title || id})),...Object.entries(model.pages).map(([id,p])=>({key:'pages/'+id+':image',file:p.image,title:p.title || id})),...Object.entries(model.settings.look.components || {}).map(([id,c])=>({key:'settings/look:components.'+id+'.image',file:c.image,title:'Custom picture'})),...Object.entries(model.settings.look.blocks || {}).map(([id,b])=>({key:'settings/look:blocks.'+id+'.image',file:b.image,title:b.title||'Page image'})),{key:'settings/look:logo.image',file:model.settings.look.logo?.image,title:'Logo'}].filter(p=>p.file);}
...
function positionToolbar(){if(!toolbar)return;const el=selectedElement();toolbar.hidden=!el || preview || !!inline;if(toolbar.hidden)return;
  const picture=el.tagName==='IMG', id=workId();
...
function multilineKey(key){return key.includes(':body.') || /\.(text|caption)$/.test(key);}
function inlineValue(current){let value=current.el.innerText.replace(/\r/g,'');return multilineKey(current.key)?value:value.replace(/\n/g,' ');}
...
function setupCanvas(){const doc=frame.contentDocument;const style=doc.createElement('style');style.textContent=`body:not(.studio-preview) [data-edit]{cursor:text}body:not(.studio-preview) img[data-edit]{cursor:pointer}body:not(.studio-preview) [data-edit]:hover{outline:1px solid #245bd7;outline-offset:3px}body:not(.studio-preview) .studio-selected{outline:2px solid #245bd7!important;outline-offset:3px}[contenteditable=true]{min-width:1ch;white-space:pre-wrap}#studio-toolbar{position:fixed;z-index:1000;display:flex;flex-wrap:wrap;gap:4px;padding:5px;background:#fff;color:#20242a;border:1px solid #dce0e5;border-radius:5px;box-shadow:0 2px 8px #0002;max-width:calc(100vw - 16px);font:12px 'Hanken Grotesk',Arial,sans-serif}#studio-toolbar button{padding:6px 8px;border:1px solid #dce0e5;border-radius:3px;background:white;color:#20242a;font:inherit}#studio-toolbar[hidden],#studio-hover[hidden]{display:none}#studio-hover{position:fixed;z-index:999;pointer-events:none;background:#245bd7;color:white;padding:2px 6px;font:12px Arial}body:not(.studio-preview) .slot a{pointer-events:none}body:not(.studio-preview) .rv{opacity:1;transform:none}body:not(.studio-preview) .slot img:not(.on){pointer-events:none}`;doc.head.append(style);
  ...
  doc.addEventListener('click',e=>{if(preview || toolbar.contains(e.target) || e.target.closest('[data-missing-picture]'))return;const thumbnail=e.target.closest('.work-overview button');const el=thumbnail?doc.querySelector('.works figure[data-i="'+thumbnail.dataset.workIndex+'"] img[data-edit]'):e.target.closest('[data-edit]');if(el){e.preventDefault();e.stopImmediatePropagation();if(inline?.el===el)return;commitInline();select(el);if(el.tagName!=='IMG')startInline(el);}else if(!e.target.closest('#studio-details')){commitInline();deselect();}},true);
...
function applyText(pending){const [record,field]=pending.key.split(':');let obj=record.split('/').reduce((o,k)=>o[k],model);const parts=field.split('.');for(const part of parts.slice(0,-1))obj=obj[part] ??= {};obj[parts.at(-1)]=pending.value;syncPageName(model,pending.key,pending.value);}
...
function detailsFields(id){return ['title','medium','size','year','price'].map(k=>field('works/'+id+':'+k,k[0].toUpperCase()+k.slice(1))).join('')+`<label>Portfolio category<select data-field="works/${esc(id)}:category">${sorted(model.categories).map(([key,c])=>`<option value="${esc(key)}" ${model.works[id].category===key?'selected':''}>${esc(c.title)}</option>`).join('')}</select></label>`+field('works/'+id+':sold','Sold','checkbox');}
...
async function preparePicture(file){if(!file || !/^image\/(png|jpeg|webp|gif)$/.test(file.type))throw Error('Choose a PNG, JPEG, WebP or GIF picture.');
  if(file.size>25*1024*1024)throw Error('This picture is over 25 MB. Save a smaller copy and try again.');
  const validationURL=URL.createObjectURL(file),testImage=new Image();testImage.src=validationURL;
  try{await testImage.decode();}catch{throw Error('This picture cannot be opened. Export it as a JPEG or PNG and try again.');}finally{URL.revokeObjectURL(validationURL);}
  const ext={ 'image/png':'png','image/jpeg':'jpg','image/webp':'webp','image/gif':'gif'}[file.type];const name=slug(file.name.replace(/\.[^.]+$/,''))+'-'+crypto.randomUUID().slice(0,8)+'.'+ext;$('#draft-status').textContent='Saving…';
...
$('#picture-file').onchange=async e=>{if(!e.target.files.length)return;try{const n=await preparePicture(e.target.files[0]);mutate(m=>{m.media.push(n);if(pictureTarget){const [r,f]=pictureTarget.split(':');let obj=r.split('/').reduce((o,k)=>o[k],m);const parts=f.split('.');for(const k of parts.slice(0,-1))obj=obj[k] ??= {};obj[parts.at(-1)]='/assets/work/'+n;}});refreshPanel();}catch(e){say(e.message);}};
